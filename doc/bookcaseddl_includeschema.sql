-- 회원
DROP TABLE IF EXISTS `bookcase`.`user` RESTRICT;

-- 중고거래게시판
DROP TABLE IF EXISTS `bookcase`.`sellboard` RESTRICT;

-- 북케이스
DROP TABLE IF EXISTS `bookcase`.`bookcase` RESTRICT;

-- 장르
DROP TABLE IF EXISTS `bookcase`.`genre` RESTRICT;

-- 책
DROP TABLE IF EXISTS `bookcase`.`book` RESTRICT;

-- 작가
DROP TABLE IF EXISTS `bookcase`.`writer` RESTRICT;

-- 리뷰
DROP TABLE IF EXISTS `bookcase`.`review` RESTRICT;

-- 북케이스내부책
DROP TABLE IF EXISTS `bookcase`.`inner_book` RESTRICT;

-- 팔로우한 작가
DROP TABLE IF EXISTS `bookcase`.`writer_like` RESTRICT;

-- 좋아한 리뷰
DROP TABLE IF EXISTS `bookcase`.`review_like` RESTRICT;

-- 거래문의
DROP TABLE IF EXISTS `bookcase`.`ask_tobuy` RESTRICT;
...
-- 중고거래게시판
CREATE TABLE `bookcase`.`sellboard` (
...
-- 중고거래게시판
ALTER TABLE `bookcase`.`sellboard`
...
-- 북케이스
CREATE TABLE `bookcase`.`bookcase` (
...
CREATE UNIQUE INDEX `UIX_review`
  ON `bookcase`.`review` ( -- 리뷰
    `book_no` ASC, -- 책번호
    `user_no` ASC  -- 작성자
  );

-- 북케이스내부책
CREATE TABLE `bookcase`.`inner_book` (
...
-- 북케이스내부책
ALTER TABLE `bookcase`.`inner_book`
...
-- 팔로우한 작가
CREATE TABLE `bookcase`.`writer_like` (
  `user_no`   INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
  `writer_no` INTEGER NOT NULL COMMENT '작가번호' -- 작가번호
)
COMMENT '팔로우한 작가';
...
-- 팔로우한 작가
ALTER TABLE `bookcase`.`writer_like`
...
-- 좋아한 리뷰
CREATE TABLE `bookcase`.`review_like` (
  `user_no`   INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
...
-- 좋아한 리뷰
ALTER TABLE `bookcase`.`review_like`
...
-- 거래문의
CREATE TABLE `bookcase`.`ask_tobuy` (
...
-- 거래문의
ALTER TABLE `bookcase`.`ask_tobuy`
...
ALTER TABLE `bookcase`.`inner_book`
  ...
  REFERENCES `bookcase`.`bookcase` ( -- 북케이스
  `no` -- 북케이스번호
  );
...
ALTER TABLE `bookcase`.`ask_tobuy`
  ...
  REFERENCES `bookcase`.`sellboard` ( -- 중고거래게시판
  `no` -- 게시글번호
  );
